...
const LoadingSpinner = () => (
    <div className="flex flex-col items-center justify-center min-h-[200px] mt-20"> {/* Added mt-20 for top spacing */}
        <div className="animate-spin h-12 w-12 border-4 border-purple-500 rounded-full border-t-transparent shadow-lg"></div> {/* Changed blue to purple-500 */}
        <p className="mt-4 text-lg text-gray-700 font-medium">Fetching your feedback...</p>
    </div>
);

const ErrorMessage = ({ message }) => (
    <div className="max-w-md mx-auto mt-20 p-6 bg-red-100 border border-red-400 text-red-700 rounded-lg shadow-md text-center animate-fade-in"> {/* Added mt-20 */}
        <p className="text-xl font-semibold mb-3">Oops! Something went wrong.</p>
        <p>{message}</p>
        <p className="mt-4 text-sm">Please try refreshing the page or contact support if the issue persists.</p>
    </div>
...
const NoFeedbackYet = () => (
    <div className="max-w-md mx-auto mt-20 p-6 bg-blue-100 border border-blue-400 text-blue-700 rounded-lg shadow-md text-center animate-fade-in"> {/* Added mt-20 */}
        <p className="text-xl font-semibold mb-3">No Feedback Available</p>
        <p>It looks like feedback for this interview hasn't been generated yet.</p>
        <p className="mt-4 text-sm">Please check back later or contact the administrator.</p>
    </div>
...
const Feedback = () => {
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8 bg-gray-50 min-h-screen pt-60"> {/* Added pt-24 here */}
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-4xl font-extrabold text-gray-800 text-center flex-grow">
                    Interview Feedback Report
                </h2>
                <button
                    onClick={handlePrint}
                    className="bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-6 rounded-lg shadow-md transition duration-300 ease-in-out transform hover:scale-105 print:hidden"
                >
                    Print Feedback
                </button>
            </div>
            ...
            {history.length > 0 && (
                <div className="mb-10 bg-white p-6 rounded-2xl shadow-md border border-purple-200"> {/* Changed border-blue-200 to border-purple-200 */}
                    <h3 className="text-2xl font-bold text-purple-700 mb-4">Progress Over Time</h3> {/* Changed text-blue-700 to text-purple-700 */}
                    <ResponsiveContainer width="100%" height={300}>
                        <LineChart data={progressData}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="count" />
                            ...
                            <Tooltip
                                ...
                                labelFormatter={(label) => `Attempt: ${label}`}
                            />
                            <Line type="monotone" dataKey="score" stroke="#8B5CF6" strokeWidth={3} /> {/* Changed stroke color to a Tailwind purple */}
                        </LineChart>
                    </ResponsiveContainer>
                </div>
            )}
            ...
            <div className="bg-white shadow-xl rounded-2xl p-8 border border-gray-200">
                ...
                <SectionToggle
                    ...
                    color="purple"  
                >
                    <p><strong>Score:</strong> <span className="text-purple-600">{feedback.overallScore || 0}/100</span></p> {/* Changed text-blue-600 to text-purple-600 */}
                    <p><strong>Summary:</strong> {feedback.detailedFeedback || 'Not provided.'}</p>
                </SectionToggle>
                ...
                {feedback.areasForImprovement?.length > 0 && (
                    ...
                    >
                        <ul className="list-disc pl-6 space-y-2">
                            {feedback.areasForImprovement.map((item, i) => (
                                <li key={i} className="bg-orange-50 p-2 rounded border border-orange-200">{item}</li> 
                ...
                {conversation.length > 0 && (
                    <div className="mt-6">
                        <button
                            ...
                            onClick={() => toggleSection('conversation')}
                        >
                            <h4 className="text-xl font-bold text-indigo-700">Conversation History</h4> {/* Changed to indigo */}
                            <span className="text-indigo-500 text-2xl"> {/* Changed to indigo */}
                                {openSections.conversation ? '−' : '+'}
                            </span>
                        </button>
                        {openSections.conversation && (
...
const SectionToggle = ({ title, isOpen, onToggle, children, color = 'blue' }) => {
    const colorMap = {
        ...
        yellow: 'text-yellow-700 border-yellow-200',
        red: 'text-red-700 border-red-200',
        purple: 'text-purple-700 border-purple-200', // Added purple
        orange: 'text-orange-700 border-orange-200', // Added orange
    };
    ...
    const textColorMap = {
        ...
        red: 'text-red-500',
        purple: 'text-purple-500', // Added purple
        orange: 'text-orange-500', // Added orange
    };
    ...
    return (
        <div className={`mb-6 pb-4 border-b ${colorMap[color]}`}>
            <button className="flex justify-between items-center w-full text-left py-2" onClick={onToggle}>
                <h3 className={`text-xl font-bold ${colorMap[color]}`}>{title}</h3>
                <span className={`${textColorMap[color]} text-2xl`}>{isOpen ? '−' : '+'}</span>
            </button>
            {isOpen && <div className="mt-4 text-gray-700 animate-fade-in-down">{children}</div>}
        </div>
    );

...
const Interview = () => {
  ...
  useEffect(() => {
    const checkAuth = () => {
      const token = localStorage.getItem('token');
      if (!token) {
        alert("Session expired or you logged out elsewhere. Redirecting to login.");
        vapi.stop(); // Stop the interview if ongoing
        navigate('/login');
      }
    };
    ...
    // Also check on storage changes (across tabs)
  ...
  //       `http://localhost:5000/api/interview/${'someInterviewId'}/feedback`, // You'll need to pass a real interviewId
  //       {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        ...
        {
          headers: {
            'x-auth-token': token, // Attach token here
          },
        }
      );

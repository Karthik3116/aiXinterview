...
const Interview = ({ user, vapi, vapiApiKey }) => {
    ...
    useEffect(() => {
        ...
        const loadInterview = async () => {
            ...
            if (interviewId && interviewId !== 'new') {
                try {
                    const workingBaseUrl = await getWorkingApiBaseUrl();
                    const res = await axios.get(
                        `${workingBaseUrl}/api/interview/${interviewId}`, {
                            headers: { 'x-auth-token': token },
                        }
                    );
                    const data = res.data;
        ...
        window.addEventListener('storage', handleStorageChange);
        return () => window.removeEventListener('storage', handleStorageChange);
    }, [navigate, interviewId, vapi, vapiApiKey]);
    ...
    const saveInterviewDataAndGenerateFeedback = useCallback(
        async (finalConversation) => {
            ...
            try {
                ...
                setLoadingProgress(30);
                await axios.post(
                    `${baseUrl}/api/interview/${currentInterviewRecordId}/complete`, { conversation: finalConversation }, { headers: { 'x-auth-token': token } }
                );
                setIsInterviewCompleted(true);
                setLoadingProgress(60);
                ...
                const updatedRes = await axios.get(
                    `${baseUrl}/api/interview/${currentInterviewRecordId}`, { headers: { 'x-auth-token': token } }
                );
                setVapiRecordingUrl(updatedRes.data.vapiRecordingUrl || null);
                setFullTranscript(updatedRes.data.fullTranscript || null);
                setLoadingProgress(100);
        ...
        [currentInterviewRecordId, subject, localUserName, navigate, baseUrl]
    );
    ...
    useEffect(() => {
        if (!vapi) {
            console.warn("Vapi instance not available. Skipping event listeners.");
            return;
        }

        const onSpeechStart = () => { if (userCaption) { addMessage('user', userCaption); } setUserCaption(''); setCurrentSpeakingRole('assistant'); };
        const onSpeechEnd = () => { if (assistantCaption) { addMessage('assistant', assistantCaption); } setAssistantCaption(''); setCurrentSpeakingRole(null); };
        const onUserSpeechStart = () => { if (assistantCaption) { addMessage('assistant', assistantCaption); } setAssistantCaption(''); setCurrentSpeakingRole('user'); };
        const onUserSpeechEnd = () => { if (userCaption) { addMessage('user', userCaption); } setUserCaption(''); setCurrentSpeakingRole(null); };

        const onCallStart = () => {
            setCallStatus('ACTIVE');
            toast.info('Vapi call started.');
            if (!currentInterviewRecordId || interviewId === 'new') { setConversation([]); }
            setAssistantCaption(''); setUserCaption(''); setCurrentSpeakingRole(null);
        };
        ...
        const onCallEnd = () => {
            ...
            toast.info('Vapi call ended. Generating feedback...');
            const finalTranscript = [...conversationRef.current];
            if (assistantCaption && assistantCaption.trim() !== '') { finalTranscript.push({ role: 'assistant', content: assistantCaption, timestamp: new Date() }); }
            if (userCaption && userCaption.trim() !== '') { finalTranscript.push({ role: 'user', content: userCaption, timestamp: new Date() }); }
            setConversation(finalTranscript);
            saveInterviewDataAndGenerateFeedback(finalTranscript);
            vapi.stop();
            setAssistantCaption(''); setUserCaption(''); setCurrentSpeakingRole(null);
        ...
        const onMessage = (msg) => {
            if (msg.type === 'transcript' && msg.transcriptType === 'final') {
                if (msg.role === 'assistant') { addMessage('assistant', msg.transcript); setAssistantCaption(''); }
                else if (msg.role === 'user') { addMessage('user', msg.transcript); setUserCaption(''); }
            } else if (msg.type === 'transcript' && msg.transcriptType === 'partial') {
                if (msg.role === 'assistant') { setAssistantCaption(msg.transcript); }
                else if (msg.role === 'user') { setUserCaption(msg.transcript); }
            }
        };
    ...
    }, [saveInterviewDataAndGenerateFeedback, addMessage, interviewId, currentInterviewRecordId, assistantCaption, userCaption, vapi]);

    const startVapiCall = async (questionsArray, currentSubject, initialConversation = []) => {
        if (!vapi) { toast.error('Vapi SDK not initialized.'); return; }
        if (!currentSubject || !questionsArray?.length) { toast.error('Cannot start: Invalid subject or questions.'); return; }

        const questionList = questionsArray.map((q, i) => `${i + 1}. ${q}`).join('\n');
        let initialMessages = [{
            role: 'system',
            content: `You are an AI voice assistant named Jennifer conducting an interview for the role of ${currentSubject}. Introduce yourself briefly. Ask only the following questions, one by one: ${questionList}. After each answer, give a brief, friendly transition like "Great, thank you." or "Alright, next question:". Do not critique answers. After the last question, say "That was the last question. Thank you for your time!" and end the call.`,
        }, ];
        if (initialConversation.length > 0) { initialMessages.push(...initialConversation.filter(msg => msg.role !== 'system')); }
        const firstMessage = initialConversation.length === 0 ? `Hi ${localUserName}, I'm Jennifer, your AI interviewer for the ${currentSubject} role. Ready to begin?` : 'Welcome back! Let\'s continue.';
        ...
        try {
            await vapi.start({
                ...
                voice: { provider: 'playht', voiceId: 'jennifer' },
                model: { provider: 'openai', model: 'gpt-4', messages: initialMessages },
                metadata: { interviewRecordId: currentInterviewRecordId },
            });
            setCallStatus('ACTIVE');
            toast.success('Vapi call started successfully!');
        } catch (err) {
    ...
    const handleCreateNewInterview = async (newSubject, newNumQuestions, jobDescription, resume) => {
        if (!vapiApiKey) {
            toast.error('Vapi API Key is required. Please add it in settings.');
            return;
        }
        setPreparingInterview(true);
        setLoadingProgress(0);
        ...
        if (!newSubject || newNumQuestions < 1) {
            toast.error('Enter subject and a valid number of questions.');
            setPreparingInterview(false);
            return;
        }
        ...
        try {
            ...
            formData.append('numQuestions', newNumQuestions);
            if (jobDescription) {
            ...
            const res = await axios.post(
                `${baseUrl}/api/interview/generate`,
                formData, { headers: { 'x-auth-token': token } }
            );
            setLoadingProgress(50);
            ...
            await startVapiCall(newQs, newSubject, []);
            setLoadingProgress(100);
            toast.success('Personalized interview created and started!');
        } catch (err) {
            console.error('Create interview error:', err);
            const errorMessage = err.response?.data?.msg || err.response?.data?.errors?.[0]?.msg || 'Failed to create interview.';
    ...
    const handleContinueInterview = async () => {
        if (!vapiApiKey) { toast.error('Vapi API Key is required.'); return; }
        if (!currentInterviewRecordId || !questions.length || isInterviewCompleted) { toast.error('No active interview to continue.'); return; }
    ...
    const handleRetakeInterview = async () => {
        if (!vapiApiKey) { toast.error('Vapi API Key is required.'); return; }
        if (!currentInterviewRecordId || !subject || numQuestions < 1) { toast.error('Cannot retake: Invalid interview data.'); return; }
        ...
        try {
            const token = localStorage.getItem('token');
            setLoadingProgress(20);
            const res = await axios.post( `${baseUrl}/api/interview/${currentInterviewRecordId}/regenerate`, { subject, numQuestions }, { headers: { 'x-auth-token': token } } );
            setLoadingProgress(50);
            ...
            console.warn('Retake with regeneration failed:', err);
            toast.warn('Regeneration failed. Retaking with existing questions.');
            setLoadingProgress(50);
            setConversation([]); setIsInterviewCompleted(false); setVapiRecordingUrl(null); setFullTranscript(null);
            try {
                await startVapiCall(questions, subject, []);
                setLoadingProgress(100);
                toast.success('Interview retaken with existing questions!');
    ...
    return (
        <div className="container mt-5 pt-4">
            <h2 className="mb-4">AI Interview Practice</h2>
            {(loading || preparingInterview) && (
                <div className="my-3">

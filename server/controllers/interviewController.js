...
// @access  Private
exports.generateQuestions = async (req, res) => {
    const { subject, numQuestions, jobDescription } = req.body;
    const userId = req.user.id; // From auth middleware
    ...
    try {
        ...
        let prompt;
        const hasContext = jobDescription || resumeText;

        if (hasContext) {
            prompt = `
        You are an expert technical recruiter generating interview questions for a "${subject}" role.
        Use the provided job description and candidate resume to create ${numQuestions} highly relevant and targeted questions.
        The questions should probe the candidate's skills and experience in relation to the job's requirements.

        ${jobDescription ? `--- JOB DESCRIPTION ---\n${jobDescription}\n` : ''}
        ${resumeText ? `--- CANDIDATE RESUME ---\n${resumeText}\n` : ''}

        Provide the output as a single, valid JSON object with a key "questions" containing an array of strings.
        Example: {"questions": ["Based on your experience with X, how would you handle Y?", "Tell me about project Z mentioned in your resume."]}
      `;
        } else {
            // Fallback to the original prompt if no context is provided
            prompt = `Generate exactly ${numQuestions} interview questions for a ${subject} role. Provide them as a JSON array of strings, like this: {"questions": ["Question 1?", "Question 2?", ...]}.`;
        }
        // --- END PROMPT BUILDING ---
        ...
        const newInterview = new Interview({
            ...
            numQuestions,
            questions,
            jobDescription: jobDescription || '', // Save to DB
            resumeText: resumeText || '',       // Save to DB
            completed: false, // Mark as not completed initially
            history: [], // Initialize empty history
        });
...
exports.generateFeedback = async (req, res) => {
    ...
    try {
        ...
        if (!feedbackData || feedbackData.overallScore === undefined) { // Basic validation
            console.error("Parsed feedback is invalid or missing required fields:", feedbackData);
            return res.status(500).json({ error: "Failed to parse structured feedback from model.", rawText });
        }

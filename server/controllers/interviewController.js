...
The candidate's name is "${candidateName}".
    ...
    {
      "overallScore": "Number between 0 and 100, representing the candidate's overall performance.",
      "strengths": ["Array of strings, listing 2-3 key strengths demonstrated."],
      "areasForImprovement": ["Array of strings, listing 2-3 key areas for improvement."],
      "detailedFeedback": "A comprehensive paragraph summarizing the interview, highlighting key aspects, and providing actionable advice."
    }
    Ensure the JSON is perfectly formatted and can be directly parsed. Do not include any other text outside the JSON block.
    `.trim();
};
...
exports.generateQuestions = async (req, res) => {
    ...
    try {
        const prompt = `Generate ${numQuestions} common interview questions for a "${subject}" role. Provide them as a JSON array of strings, e.g., ["Question 1", "Question 2"]. Do not include any other text or formatting.`;
        const result = await model.generateContent(prompt);
        const responseText = result.response.text();
        const parsedQuestions = parseJsonFromText(responseText);

        if (!Array.isArray(parsedQuestions)) {
            console.error('AI response was not a JSON array:', responseText);
            return res.status(500).json({ msg: 'Failed to generate questions in expected format.' });
        }
        ...
        await newInterview.save();
        res.json({ questions: newInterview.questions, interviewId: newInterview._id });

    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error generating questions or creating interview.');
    }
};

// @route   POST api/interview/:id/regenerate
// @desc    Regenerate interview questions for an existing interview record
// @access  Private
exports.regenerateQuestions = async (req, res) => {
    const { id } = req.params;
    const { subject, numQuestions } = req.body; // Expect subject and numQuestions to be sent
    const userId = req.user.id;
    ...
    try {
        ...
        // Ensure the user owns this interview record
        if (interview.userId.toString() !== userId) {
            return res.status(401).json({ msg: 'User not authorized' });
        }

        const prompt = `Generate ${numQuestions} new interview questions for a "${subject}" role. Provide them as a JSON array of strings, e.g., ["Question 1", "Question 2"]. Do not include any other text or formatting.`;
        const result = await model.generateContent(prompt);
        const responseText = result.response.text();
        const newQuestions = parseJsonFromText(responseText);

        if (!Array.isArray(newQuestions)) {
            console.error('AI response for regeneration was not a JSON array:', responseText);
            return res.status(500).json({ msg: 'Failed to regenerate questions in expected format.' });
        }

        // Update the existing interview record
        interview.questions = newQuestions;
        interview.conversation = []; // Clear conversation for a fresh retake
        interview.completed = false; // Mark as not completed
        interview.vapiRecordingUrl = null; // Clear previous recording URL
        interview.fullTranscript = null; // Clear previous full transcript
        // You might want to increment a retake count or add to history here if applicable
        ...
        console.error(err.message);
        res.status(500).send('Server Error regenerating questions.');
    }
};
...
exports.getAllInterviews = async (req, res) => {
    try {
        ...
        console.error(err.message);
        res.status(500).send('Server Error fetching interviews');
    }
};
...
exports.getInterviewById = async (req, res) => {
    try {
        ...
        res.status(500).send('Server Error fetching interview');
    }
};

// @route   POST api/interview/:id/complete
// @desc    Complete an interview (save final conversation, mark as completed)
// @access  Private
exports.completeInterview = async (req, res) => {
    const { conversation } = req.body;
    const userId = req.user.id;
    ...
    try {
        ...
        interview.conversation = conversation;
        interview.completed = true; // Mark as completed
        // vapiRecordingUrl and fullTranscript will be updated by the webhook

        await interview.save();
        res.json({ msg: 'Interview completed and conversation saved.', interview });

    } catch (err) {
        console.error(err.message);
        res.status(500).send('Server Error completing interview');
    }
};

// @route   POST api/interview/:interviewId/feedback
// @desc    Generate and save feedback for a specific interview
// @access  Private
exports.generateFeedback = async (req, res) => {
    const { conversation, jobRole, candidateName } = req.body;
    ...
    try {
        ...
        // Check if the interview is completed before generating feedback
        if (!interview.completed) {
            return res.status(400).json({ msg: 'Interview must be completed to generate feedback.' });
        }

        const prompt = generateFeedbackPrompt(conversation, jobRole, candidateName);
        const result = await model.generateContent(prompt);
        const responseText = result.response.text();
        const generatedFeedback = parseJsonFromText(responseText);

        if (!generatedFeedback || !generatedFeedback.overallScore || !Array.isArray(generatedFeedback.strengths) || !Array.isArray(generatedFeedback.areasForImprovement) || !generatedFeedback.detailedFeedback) {
            console.error('AI feedback was not in expected JSON format:', responseText);
            return res.status(500).json({ msg: 'Failed to generate feedback in expected format.' });
        }

        // Update the interview record with the new feedback
        interview.feedback = {
            ...
            detailedFeedback: generatedFeedback.detailedFeedback,
        };

        // Add to history
        const newRetakeNumber = (interview.history.length > 0 ? interview.history[interview.history.length - 1].retakeNumber + 1 : 1);
        interview.history.push({
            overallScore: generatedFeedback.overallScore,
            timestamp: new Date(),
        ...
        res.json({ msg: 'Feedback generated and saved successfully.', feedback: interview.feedback });

    } catch (err) {
        console.error('Error generating feedback:', err.message);
        res.status(500).send('Server Error generating feedback.');
    }
};
...
exports.getInterviewFeedback = async (req, res) => {
    ...
    try {
        ...
        if (!interview.feedback || Object.keys(interview.feedback).length === 0) {
            return res.status(404).json({ msg: 'Feedback not found for this interview.' });
        }

        res.json({ feedback: interview.feedback, history: interview.history, vapiRecordingUrl: interview.vapiRecordingUrl, fullTranscript: interview.fullTranscript });
    } catch (err) {
        console.error(err.message);
        if (err.kind === 'ObjectId') {
            return res.status(404).json({ msg: 'Interview not found' });
        }
        res.status(500).send('Server Error fetching interview feedback');
    }
};

// NEW: Handler for Vapi webhooks
// @route   POST api/interview/vapi-webhook
// @desc    Handles incoming Vapi webhook events, primarily call-end to save recording and transcript
// @access  Public (Vapi's servers will call this)
exports.handleVapiWebhook = async (req, res) => {
    const { event, call, customData } = req.body;

    console.log('Received Vapi Webhook Event:', event);
    // You can uncomment these for more detailed logging during development:
    // console.log('Call data:', JSON.stringify(call, null, 2));
    // console.log('Custom data:', JSON.stringify(customData, null, 2));

    try {
        if (event === 'call-end') {
            // Vapi sends the metadata passed during `vapi.start()` in `call.metadata`
            const interviewRecordId = call?.metadata?.interviewRecordId || customData?.interviewRecordId; // Check both in case of Vapi payload changes

            if (!interviewRecordId) {
                console.warn('Vapi Webhook: Missing interviewRecordId in metadata/customData for call-end event.');
                return res.status(400).json({ msg: 'Missing interviewRecordId' });
            }

            // Validate if interviewRecordId is a valid MongoDB ObjectId
            if (!mongoose.Types.ObjectId.isValid(interviewRecordId)) {
                console.warn('Vapi Webhook: Invalid interviewRecordId received:', interviewRecordId);
                return res.status(400).json({ msg: 'Invalid interviewRecordId' });
            }

            const interview = await Interview.findById(interviewRecordId);

            if (!interview) {
                console.warn(`Vapi Webhook: Interview record not found for ID: ${interviewRecordId}`);
                return res.status(404).json({ msg: 'Interview record not found' });
            }

            // Extract recording URL and full transcript from the Vapi call-end payload
            const vapiRecordingUrl = call?.publicAudioUrl; // This is the URL Vapi provides
            const fullTranscript = call?.transcript;       // This is the full transcript from Vapi

            // Update the interview record only if new data is present from Vapi
            if (vapiRecordingUrl) {
                interview.vapiRecordingUrl = vapiRecordingUrl;
            }
            if (fullTranscript) {
                interview.fullTranscript = fullTranscript;
            }

            await interview.save();
            console.log(`Successfully updated interview ${interviewRecordId} with Vapi recording URL and transcript.`);

            res.status(200).json({ msg: 'Webhook processed successfully' });

        } else {
            // Acknowledge other Vapi events, or handle them if needed
            console.log(`Vapi Webhook: Received unhandled event type: ${event}`);
            res.status(200).json({ msg: `Event type ${event} received but not processed by this handler.` });
        }
    } catch (error) {
